...
public class DefaultMessageHandler extends AbstractMethodReceiver implements MessageTarget {
	 ...
	 */
	public DefaultMessageHandler(MessageHandler info) {
		super(info);
	...
	public void handleMessage(MessageProcessor processor) {
		try {
			...
			if (messageFields == null) {
				...
				if (count >= 1) {
					if (targetMethod.getParameterTypes()[0].equals(MessageProcessor.class)) {
						args.add(processor);
					} else {
			...
			targetMethod.invoke(objectProvider.get(), args);
		}
		catch (Exception e) {
			throw new Error("Message type " + type.getClass() + " cannot be processed.", e);
		}
	}
}
